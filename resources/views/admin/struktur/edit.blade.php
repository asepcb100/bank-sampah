...
@section('title', 'Edit Struktur — Admin')

@section('page-heading', 'Edit Struktur Organisasi')

@section('content')
    <div class="max-w-full mx-auto space-y-6">

        <div class="flex items-center justify-between border-b border-[#2b2417]/10 pb-4">
            <p class="text-xs text-[#6b6150]">Perbarui rincian pengurus inti atau divisi.</p>
        ...
        <form method="POST" action="{{ route('admin.struktur.update', $item->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-8 space-y-6">
                    <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 sm:p-8 shadow-sm space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            <div class="grid gap-1.5">
                                ...
                                <select name="tipe" required class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs">
                                    <option value="inti" @selected(old('tipe', $item->tipe) === 'inti')>Pengurus Inti</option>
                                    <option value="divisi" @selected(old('tipe', $item->tipe) === 'divisi')>Divisi</option>
                                </select>
                            </div>
                            ...
                            <div class="grid gap-1.5">
                                ...
                                <select name="badge" class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs">
                                    <option value="moss" @selected(old('badge', $item->badge) === 'moss')>Moss (hijau)</option>
                                    <option value="ochre" @selected(old('badge', $item->badge) === 'ochre')>Ochre (oranye)</option>
                                </select>
                            </div>
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $item->nama) }}" required placeholder="Untuk divisi, isi nama divisinya"
                                class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs" />
                        </div>

                        <div class="grid gap-1.5">
                            <label class="text-xs font-bold text-[#2c3821]">Jabatan</label>
                            <input type="text" name="jabatan" value="{{ old('jabatan', $item->jabatan) }}" placeholder="misal: Ketua Komunitas"
                                class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs" />
                        </div>
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <input type="text" name="anggota" value="{{ old('anggota', $item->anggota) }}" placeholder="misal: Atin Indriawati & Ine Nuraini"
                                class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs" />
                        </div>
                    </div>
                </div>
