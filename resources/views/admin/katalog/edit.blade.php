...
@section('page-heading', 'Edit Produk Katalog')
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-[#2b2417]/10 pb-4">
        <p class="text-xs text-[#6b6150]">Perbarui rincian nama produk, kategori, harga, stok, dan media katalog.</p>
    ...
    <form method="POST" action="{{ route('admin.katalog.update', $product->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 sm:p-8 shadow-sm space-y-5">
                    ...
                    <div class="grid gap-1.5">
                        ...
                        <textarea name="description" 
                                  ...
                                  class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
                        ...
                        <div class="flex-1 flex items-center gap-3 overflow-x-auto py-2 scrollbar-none" id="mediaThumbnailsRow">
                            @foreach ($product->images as $image)
                                <div class="relative group w-28 h-28 sm:w-32 sm:h-32 rounded-2xl border-2 {{ $image->is_primary ? 'border-emerald-500' : 'border-slate-200 hover:border-slate-300' }} overflow-hidden bg-white shrink-0 p-0.5 transition-all shadow-2xs media-card-item" 
                                    ...
                                    <img src="{{ $image->image_url }}" alt="Media Produk" class="w-full h-full object-contain rounded-xl bg-slate-50" />
...
function setPrimaryImage(imageId) {
    ...
    const targetCard = document.getElementById('product-img-card-' + imageId);
    if (targetCard) {
...
function removeExistingImage(imageId) {
    const card = document.getElementById('product-img-card-' + imageId);
