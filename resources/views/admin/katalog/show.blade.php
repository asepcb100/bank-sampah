...
@section('page-heading', 'Detail Produk Katalog')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#2b2417]/10 pb-4">
        <a href="{{ route('admin.katalog') }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full border border-[#2b2417]/16 text-xs font-bold text-[#2b2417] hover:bg-[#f6f1e2] transition-colors shrink-0">
            ← Kembali ke Katalog
        </a>

        <div class="flex items-center gap-2.5">
            <a href="{{ route('admin.katalog.edit', $product->id) }}" class="px-4 py-2 rounded-full bg-[#c1852c] text-white hover:bg-[#a67022] text-xs font-bold transition-all shadow-xs flex items-center gap-1.5">
                <span>✏️</span>
                <span>Edit Produk</span>
            </a>
            <a href="{{ route('katalog.show', $product->slug) }}" target="_blank" class="px-4 py-2 rounded-full bg-[#2c3821] text-[#fbf8ef] hover:bg-[#4c5c31] text-xs font-bold transition-all shadow-xs flex items-center gap-1.5">
                <span>🌐</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 space-y-5">
                ...
                <div class="flex items-center gap-2.5 flex-wrap">
                    @if($product->category)
                        <span class="px-3 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300/60 text-[10px] font-extrabold uppercase tracking-wider">
                            {{ $product->category->name }}
                        </span>
                    @endif

                    <span class="px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider {{ $product->is_available ? 'bg-emerald-100 text-emerald-800 border border-emerald-300/60' : 'bg-rose-100 text-rose-800 border border-rose-300/60' }}">
                ...
                <div class="space-y-1">
                    ...
                    <p class="font-fraunces font-bold text-xl text-[#c1852c]">
                        {{ $product->price_text ?: 'Hubungi kami' }}
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
                    <div class="p-3.5 bg-white rounded-2xl border border-[#2b2417]/12 shadow-2xs flex items-center gap-3">
                        ...
                        <div>
                            <span class="block text-[10px] text-slate-400 font-bold uppercase tracking-wider">Jumlah Stok</span>
                            <span class="text-xs font-bold text-[#2c3821]">{{ $product->stock }} unit</span>
                        </div>
                    </div>
                    ...
                    <div class="p-3.5 bg-white rounded-2xl border border-[#2b2417]/12 shadow-2xs flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-[#f6f1e2] text-[#c1852c] flex items-center justify-center font-bold text-sm shrink-0">👤</div>
                        <div>
                            <span class="block text-[10px] text-slate-400 font-bold uppercase tracking-wider">PIC Pemesanan WA</span>
                            <span class="text-xs font-bold text-[#2c3821]">{{ $product->contact?->name ?? '-' }} ({{ $product->contact?->phone ?? '-' }})</span>
                        </div>
                    </div>
                </div>
                ...
                @if($product->description)
                    <div class="space-y-2 pt-3 border-t border-[#2b2417]/10">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-[#c1852c]">Rincian & Deskripsi Produk</h4>
            ...
            <div class="lg:col-span-5 space-y-4">
                ...
                <div class="flex items-center justify-between border-b border-[#2b2417]/10 pb-2">
                    ...
                    <span class="text-[11px] text-slate-500 font-semibold">{{ $product->images->count() }} Foto</span>
                </div>
                ...
                <div class="relative group h-48 sm:h-56 w-full rounded-2xl border border-[#2b2417]/16 overflow-hidden bg-white shadow-2xs cursor-pointer"
                     onclick="openImageZoom('{{ $product->image_url }}', 'Foto Utama - {{ addslashes($product->title) }}')">
                    
                    <img src="{{ $product->image_url }}" alt="{{ $product->title }}" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500 bg-slate-50 p-2" />
                    ...
                    <div class="grid grid-cols-4 sm:grid-cols-5 gap-2.5 pt-1">
                        @foreach ($product->images as $img)
                            <div class="relative group aspect-square rounded-xl border border-slate-200 overflow-hidden bg-white cursor-pointer shadow-2xs p-0.5 transition-all hover:border-[#c1852c]"
                                 onclick="openImageZoom('{{ $img->image_url }}', 'Dokumentasi {{ addslashes($product->title) }}')">
                                <img src="{{ $img->image_url }}" alt="Media" class="w-full h-full object-contain rounded-lg group-hover:scale-110 transition-transform duration-300 bg-slate-50" />
...
<div id="imageZoomModal" 
    ...
    <div class="relative max-w-4xl max-h-[85vh] w-full flex flex-col items-center justify-center space-y-3" onclick="event.stopPropagation()">
        <img id="zoomedImageDisplay" src="" alt="Zoomed Media" class="max-w-full max-h-[75vh] object-contain rounded-2xl shadow-2xl border border-white/20 bg-white/10 p-2" />
