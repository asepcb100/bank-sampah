...
@section('title', 'Edit Kelompok Binaan — Admin')

@section('page-heading', 'Edit Kelompok Binaan')
...
@section('content')
    <div class="max-w-full mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between border-b border-[#2b2417]/10 pb-4">
            <p class="text-xs text-[#6b6150]">Perbarui informasi kelompok binaan, alamat, tanggal berdiri, pengurus, dan
                kontak WhatsApp.</p>
        ...
        <form method="POST" action="{{ route('admin.binaan.update', $binaan->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 sm:p-8 shadow-sm space-y-5">
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $binaan->nama) }}" required
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <textarea name="alamat" rows="2" placeholder="misal: RT 04 / RW 02, Desa Sindang, Kec. Sindang – Indramayu"
                                class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs">{{ old('alamat', $binaan->alamat) }}</textarea>
                        </div>

                        <!-- Tanggal Berdiri -->
                        <div class="grid gap-1.5">
                            <label class="text-xs font-bold text-[#2c3821]">Berdiri Sejak</label>
                            <input type="date" name="berdiri_sejak"
                                value="{{ old('berdiri_sejak', $binaan->berdiri_sejak?->format('Y-m-d')) }}"
                                class="w-full px-4 py-3 bg-white border border-[#2b2417]/16 rounded-2xl text-xs sm:text-sm font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c] focus:border-[#c1852c] transition-all shadow-2xs" />
                        </div>
                    </div>
                    ...
                    <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 sm:p-8 shadow-sm space-y-5">
                        ...
                        <div id="pengurusan-rows" class="space-y-3">
                            @forelse ($binaan->pengurusan as $i => $pengurus)
                                <div
                                    ...
                                    <div class="flex-1 w-full grid gap-1">
                                        ...
                                        <input type="text" name="pengurusan[{{ $i }}][nama]"
                                            value="{{ old('pengurusan.' . $i . '.nama', $pengurus->nama) }}"
                                    ...
                                    <div class="flex-1 w-full grid gap-1">
                                        ...
                                        <input type="text" name="pengurusan[{{ $i }}][jabatan]"
                                            value="{{ old('pengurusan.' . $i . '.jabatan', $pengurus->jabatan) }}"
                    ...
                    <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 sm:p-8 shadow-sm space-y-5">
                        ...
                        <div id="kontak-rows" class="space-y-3">
                            @forelse ($binaan->kontak as $i => $kontak)
                                <div
                                    ...
                                    <div class="flex-1 w-full grid gap-1">
                                        ...
                                        <input type="text" name="kontak[{{ $i }}][nama]"
                                            value="{{ old('kontak.' . $i . '.nama', $kontak->nama) }}"
                                            placeholder="Nama kontak"
                                            class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-[#2c3821] focus:outline-none focus:ring-2 focus:ring-[#c1852c]" />
                                    </div>
                                    <div class="flex-1 w-full grid gap-1">
                                        <label class="text-[10px] font-bold text-[#6b6150]">Nomor WhatsApp</label>
                                        <input type="text" name="kontak[{{ $i }}][whatsapp]"
                                            value="{{ old('kontak.' . $i . '.whatsapp', $kontak->whatsapp) }}"
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-[#fbf8ef] rounded-3xl border border-[#2b2417]/16 p-6 shadow-sm space-y-5">
                        <h3 class="font-fraunces font-bold text-lg text-[#2c3821] border-b border-[#2b2417]/10 pb-3">Aksi
                            Simpan</h3>
                        <p class="text-xs text-[#6b6150]">Perubahan data pengurus & kontak akan otomatis diperbarui saat
                            disimpan.</p>
